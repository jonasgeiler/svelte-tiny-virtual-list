...
<script>
  ...
  function refresh() {
    ...
    const hasStickyIndices =
      stickyIndices != null && stickyIndices.length !== 0;
    if (hasStickyIndices) {
  ...
  function getStyle(index, sticky) {
    ...
    if (scrollDirection === DIRECTION.VERTICAL) {
      ...
      if (sticky) {
        style += `position:sticky;flex-grow:0;z-index:1;top:0;margin-top:${offset}px;margin-bottom:${-(
          offset + size
        )}px;`;
      } else {
      ...
      if (sticky) {
        style += `position:sticky;z-index:1;left:0;margin-left:${offset}px;margin-right:${-(
          offset + size
        )}px;`;
      } else {
